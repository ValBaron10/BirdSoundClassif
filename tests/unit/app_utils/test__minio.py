...
from app.app_utils.minio import (
...
def test_write_file_to_minio_bytes(mock_minio_client):
    ...
    write_file_to_minio(
        mock_minio_client, "test_bucket", "test_file.txt", b"test content"
    )
...
def test_fetch_file_from_minio_success(mock_minio_client):
    # Call the function
    result = fetch_file_from_minio(
        mock_minio_client, "test_bucket", "test_file.txt", "/local/path/test_file.txt"
    )

    assert result is True
    mock_minio_client.fget_object.assert_called_once_with(
        "test_bucket", "test_file.txt", "/local/path/test_file.txt"
    )
