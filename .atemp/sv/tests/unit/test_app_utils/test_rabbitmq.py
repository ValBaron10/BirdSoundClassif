...
def test_process_feedback_message():
    ...
    local_file_path = "/tmp/tmp8d83nm43"
    with patch("app_utils.smtplib.send_email") as mock_send_email:
        process_feedback_message(body.encode(), local_file_path)
...
@pytest.mark.asyncio
async def test_consume_feedback_messages(rabbit_channel):
    ...
    feedback_queue = "test_queue"
    local_file_path = "/tmp/tmp8d83nm43"
    with patch("app_utils.rabbitmq.process_feedback_message") as mock_process_feedback_message:
        rabbit_channel.basic_get.return_value = (Mock(), None, body.encode())
        await consume_feedback_messages(rabbit_channel, feedback_queue, local_file_path)
        mock_process_feedback_message.assert_called_once_with(body.encode(), local_file_path)
