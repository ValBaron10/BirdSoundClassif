...
def patch_mocks(monkeypatch, mock_minio_client, mock_rabbitmq_channel):
    ...
    monkeypatch.setattr("app_utils.minio.ensure_bucket_exists", mock_ensure_bucket_exists)
    monkeypatch.setattr("app_utils.minio.write_file_to_minio", mock_write_file_to_minio)
    monkeypatch.setattr("app_utils.rabbitmq.get_rabbit_connection", MagicMock(return_value=MagicMock(channel=MagicMock(return_value=mock_rabbitmq_channel))))
    monkeypatch.setattr("app_utils.rabbitmq.publish_message", mock_publish_message)

    return mock_minio_client, mock_rabbitmq_channel, mock_write_file_to_minio, mock_publish_message, mock_ensure_bucket_exists

@pytest.mark.asyncio
async def test_upload_record(mock_upload_file, patch_env_vars, patch_mocks):
    mock_minio_client, mock_rabbitmq_channel, mock_write_file_to_minio, mock_publish_message, mock_ensure_bucket_exists = patch_mocks
    ...
    mock_write_file_to_minio.assert_called_once_with(mock_minio_client, "test_bucket", "test.wav", file_content)

    # Verify that the publish_message function was called correctly
    mock_publish_message.assert_called_once_with(mock_rabbitmq_channel, "forwarding_queue", {
