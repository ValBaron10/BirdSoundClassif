...
from app_utils.minio import (
    ...
    fetch_file_contents_from_minio,
    write_file_to_minio,
)
...
def initialize_clients():
    """Ensure the clients are initialized after env variables are fetched (test issues)."""
...
async def startup_event() -> None:
    ...
    It creates a task to fetch the file path using MinIO,
    consume feedback messages using the local file path,
    and delete the temporary file after processing.
                ...
                if local_file_path:
                    try:
                        await consume_feedback_messages(
                            rabbitmq_channel, FEEDBACK_QUEUE, local_file_path
                        )
                    finally:
                        os.remove(local_file_path)  # Delete the temporary file
